...
import { BackendConnectionStatus, ModelInfo } from '../api/types';
...
interface CoreState {
  ...
  fetchModelsFromBackend: () => Promise<void>;
...
export const useCoreStore = create<CoreState>()(
  persist(
    (set) => ({
      ...
      fetchModelsFromBackend: async () => {
        try {
          const apiUrl = `http://${window.location.hostname}:6400`;
          ...
          if (data.models && data.models.length > 0) {
            set({ availableModels: data.models.map((m: { id: string }) => m.id) });
          }
        } catch {
          // Keep fallback from AVAILABLE_MODELS.text
        }
      },
